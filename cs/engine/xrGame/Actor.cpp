...
void CActor::RepackAmmo()
{
	...
	shared_str asect = _ammo[0]->m_section_id; // текущая секция
	...
	_ammo.erase(remove_if(_ammo.begin(), _ammo.end(), remove_predikat(asect)), _ammo.end());
	}// while
}

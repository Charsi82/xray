...
/*
string.split( string, separator, mode, trim)
Аргумнты:
string - обрабатываемая строка

*separator - разделитель (первый символ переданной строки). по умолчанию ','

*mode:
-true/1 '[значение]=true'
-false/0 '[idx] = значение'
по умолчанию 0

*trim - обрезать пробельные символы(true\false). по умолчанию true
*/
int str_split(lua_State *L){
	...
	if (lua_gettop(L) > 1)
	{
		const char* _sep = luaL_checkstring(L, 2);
		if (_sep) sep =  _sep[0];
	}
	...
	bool trim = true;
	if (lua_gettop(L) > 3)
		trim = !!luaL_checkint(L, 4);

	//create table
	lua_newtable(L);

	//process
	int ModesCount = _GetItemCount(s, sep);
	switch (mode)
	{
	case 0:
	{
		...
		{
			string256 sItem;
			_GetItem(s, i, sItem, sep, "", trim);

			lua_pushnumber(L, i + 1);
			lua_pushstring(L, sItem);
			lua_settable(L, -3);
		};
		break;
	};

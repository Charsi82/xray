...
//	using CUISimpleWindow::Init;
	...
	////////////////////////////////////
	//ðàáîòà ñ äî÷åðíèìè è ðîäèòåëüñêèìè îêíàìè
	...
	//ïîëó÷èòü îêíî ñàìîãî âåðõíåãî óðîâíÿ
	...
	//ïîäíÿòü íà âåðøèíó ñïèñêà âûáðàííîå äî÷åðíåå îêíî
	bool					BringToTop			(CUIWindow* pChild);

	//ïîäíÿòü íà âåðøèíó ñïèñêà âñåõ ðîäèòåëåé îêíà è åãî ñàìîãî
	void					BringAllToTop		();
	...
	//çàõâàòèòü/îñâîáîäèòü ìûøü îêíîì
	//ñîîáùåíèå ïîñûëàåòñÿ äî÷åðíèì îêíîì ðîäèòåëüñêîìó
	void					SetCapture			(CUIWindow* pChildWindow, bool capture_status);
	CUIWindow*				GetMouseCapturer	()													{return m_pMouseCapturer;}

	//îêîøêî, êîòîðîìó ïåðåñûëàþòñÿ ñîîáùåíèÿ,
	//åñëè NULL, òî øëåì íà GetParent()
	...
	//îáðàáîòêà ñîîáùåíèé íå ïðåäóñìîòðåíûõ ñòàíäàðòíûìè îáðàáîò÷èêàìè
	//ô-öèÿ äîëæíà ïåðåîïðåäåëÿòüñÿ
	//pWnd - óêàçàòåëü íà îêíî, êîòîðîå ïîñëàëî ñîîáùåíèå
	//pData - óêàçàòåëü íà äîïîëíèòåëüíûå äàííûå, êîòîðûå ìîãóò ïîíàäîáèòüñÿ
	virtual void			SendMessage			(CUIWindow* pWnd, s16 msg, void* pData = NULL);
	
	

	//çàïðåùåíèå/ðàçðåøåíèå íà ââîä ñ êëàâèàòóðû
	virtual void			Enable				(bool status)									{m_bIsEnabled=status;}
	virtual bool			IsEnabled			()												{return m_bIsEnabled;}

	//óáðàòü/ïîêàçàòü îêíî è åãî äî÷åðíèå îêíà
	...
	IC void					GetAbsolutePos		(Fvector2& p) 	{Frect abs; GetAbsoluteRect(abs); p.set(abs.x1,abs.y1);}
			...
			Frect			GetWndRect_script()													{ return CUISimpleWindow::GetWndRect(); };
			void			Init(float x, float y, float w, float h)							{ m_wndPos.set(x, y); m_wndSize.set(w, h); };
			
	//ïðîðèñîâêà îêíà
	virtual void			Draw				();
	virtual void			Draw				(float x, float y);
	//îáíîâëåíèå îêíà ïåðåäïðîðèñîâêîé
	virtual void			Update				();
	...
	//äëÿ ïåðåâîäà îêíà è ïîòîìêîâ â èñõîäíîå ñîñòîÿíèå
	virtual void			Reset				();
			void			ResetAll			();
...
protected:
	...
	//ñïèñîê äî÷åðíèõ îêîí
	WINDOW_LIST				m_ChildWndList;
	
	//óêàçàòåëü íà ðîäèòåëüñêîå îêíî
	CUIWindow*				m_pParentWnd;

	//äî÷åðíåå îêíî êîòîðîå, çàõâàòèëî ââîä ìûøè
	CUIWindow*				m_pMouseCapturer;
	
	//êòî èçíà÷àëüíî èíèöèðîâàë
	//çàõâàò ôîêóñà, òîëüêî îí òåïåðü
	//ìîæåò âåñü ôîêóñ è îñâîáîäèòü
	CUIWindow*				m_pOrignMouseCapturer;

	//äî÷åðíåå îêíî êîòîðîå, çàõâàòèëî ââîä êëàâèàòóðû
	...
	// Ïîñëåäíÿÿ ïîçèöèÿ ìûøêè
	Fvector2 cursor_pos;

	//âðåìÿ ïðîøëîãî êëèêà ìûøêè
	//äëÿ îïðåäåëåíèÿ DoubleClick
	u32						m_dwLastClickTime;
	u32						m_dwFocusReceiveTime;

	//ôëàã àâòîìàòè÷åñêîãî óäàëåíèÿ âî âðåìÿ âûçîâà äåñòðóêòîðà
	bool					m_bAutoDelete;

	bool					m_bPP;
	//ðàçðåøåí ëè ââîä ïîëüçîâàòåëÿ
	bool					m_bIsEnabled;

	// Åñëè êóðñîð íàä îêíîì
